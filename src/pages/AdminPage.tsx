...
import ResetPasswordModal from "../components/ResetPasswordModal"; // ✅ استيراد المودال
...
export default function AdminPage({ onNavigate }: AdminPageProps) {
  ...
  const [receipts, setReceiptsState] = useState(getReceipts());
  ...
  const handleApproveReceipt = (receipt: Receipt) => {
    ...
    // المبلغ القابل للخصم (لا يتجاوز الدين المتبقي)
    ...
    // إعادة حساب الدين للتأكد
    recalculatePharmacyDebt(receipt.pharmacyId);
    ...
    addNotification(`✅ تم قبول إيصال من ${receipt.pharmacyName} بقيمة ${formatCurrency(maxDeductible)}`, "success");
  };

  const handleRejectReceipt = (receipt: Receipt) => {
    const updated = receipts.map((r) =>
    ...
    addNotification(`❌ تم رفض إيصال من ${receipt.pharmacyName} بقيمة ${formatCurrency(receipt.amount)}`, "warning");
  };

  const handleUpdateOrderStatus = (orderId: string, newStatus: "pending" | "delivered") => {
    ...
    addNotification(`📦 تم تحديث حالة الطلب #${orderId} إلى ${newStatus === "delivered" ? "تم التسليم" : "قيد التنفيذ"}`, "info");
  };
  ...
  const handleAddProduct = (e: React.FormEvent) => {
    ...
    addNotification(`💊 تم إضافة منتج جديد: ${newProduct.name}`, "success");
  ...
  const handleStockMovement = (e: React.FormEvent) => {
    ...
    addStockMovement(stockMovement.medicineId, stockMovement.type, stockMovement.quantity, stockMovement.note);
...
{receiptModal && (
  <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
    <div className="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] shadow-2xl overflow-y-auto">
      ...
      <div className="p-6">
        ...
        {receiptModal.status === "pending" ? (
          <div className="flex gap-3 sticky bottom-0 bg-white pb-2 pt-3 border-t border-slate-100">
            <button
